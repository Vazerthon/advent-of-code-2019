class DayThree
  ...
  def buildCoordinateList(instruction, coords)
    case instruction[0]
    when 'R'
      coords << [coords[-1][0] + 1, coords[-1][1]]
    when 'L'
      coords << [coords[-1][0] - 1, coords[-1][1]]
    when 'U'
      coords << [coords[-1][0], coords[-1][1] + 1]
    when 'D'
      coords << [coords[-1][0], coords[-1][1] - 1]
    end
  ...
  def toFullCoordinateList(list)
    coords = [[0,0]]
    list.each do |instruction|
      instruction[1..-1].to_i.times do
        coords = buildCoordinateList(instruction, coords)
      end
    end
  ...
  def findIntersections(a, b)
    frequencies = {}
    a.each { |coord| frequencies[coord] = 0}
    b.each { |coord| frequencies[coord] ? frequencies[coord] += 1 : 0}
  ...
  def first
    lineOneCoords = toFullCoordinateList @@data[0]
    lineTwoCoords = toFullCoordinateList @@data[1]
    intersections = findIntersections(lineOneCoords, lineTwoCoords)
    
    puts intersections[1..-1]
  ...
  def second
    ...
    x = intersections[1..-1].map do |intersection|
      intersection
    end
