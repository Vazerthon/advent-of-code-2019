class DayThree
  ...
  def toCoordinateList(list)
    coords = [[0,0]]
    list.each do |instruction|
      case instruction[0]
      when 'R'
        coords << [coords[-1][0] + instruction[1..-1].to_i, coords[-1][1]]
      when 'U'
        coords << [coords[-1][0], coords[-1][1] + instruction[1..-1].to_i]
      when 'D'
        coords << [coords[-1][0], coords[-1][1] - instruction[1..-1].to_i]
      when 'L'
        coords << [coords[-1][0] - instruction[1..-1].to_i, coords[-1][1]]
  ...
  def findIntersections(a, b)
    ...
    a.each { |coord| frequencies[coord] += 1}
    b.each { |coord| frequencies[coord] += 1}

    frequencies.each { |k, v| if v > 1
      puts "#{k[0]}, #{k[1]}"
    end }
  end

  def first
    lineOneCoords = toCoordinateList @@data[0]
    # lineTwoCoords = toCoordinateList @@data[1]
    # findIntersections(lineOneCoords, lineTwoCoords)
  end
